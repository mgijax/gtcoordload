...
def run():
    ...
    while line != "":
	tokenList =  string.splitfields(line, TAB)
	...
	if tokenList[2] != 'gene':
	    line = sys.stdin.readline()
	    continue
	chr = string.strip(tokenList[0])
	# strip off the leading 'chr'
	chr = chr[3:]
	start = string.strip(tokenList[3])
	end = string.strip(tokenList[4])
	strand = string.strip(tokenList[6])
	# strip of leading "Gene "
	id = string.strip(tokenList[8][4:])
